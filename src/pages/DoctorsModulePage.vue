...
export default defineComponent({
  ...
  methods: {
    ...
    async segregatePatientList(patientList) {
      ...
      return {
        forConsultation: patientList.filter(
          (filterForConsultation) => filterForConsultation.encounterStatus === 1
        ),
        completed: patientList.filter(
          (filterCompleted) => filterCompleted.encounterStatus === 2
        ),
      };
    },
